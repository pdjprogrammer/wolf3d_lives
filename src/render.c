...
void R_DrawFace(void)
{
	...
	if(Player.health)
	{
		if(Player.face_gotgun)
			...
			// Have to maintain proper face at health above 100
			face = FACE1APIC + 3 * ((100 - 100) >> 4) + Player.faceframe;
		else
...
void R_DrawHUD(int bonusflash)
{
	...
	R_LatchNumber(224, 422, 2, Player.lives);
	R_LatchNumber(336, 422, 3, Player.health);
	R_LatchNumber(432, 422, 3, Player.ammo[AMMO_BULLETS]);
}
...
void R_Draw2D(void)
{
	if(M_Draw3D())
	{
		...
		if(r_automap)
			AM_DrawAutomap(r_automap_transparent);
		if(developer->value)
			DrawDebug();
	}
...
void R_FrameCounter(void)
{
	...
	if(!fps_timer_old)
		fps_timer_old = timeGetTime();

	frames++;
	fps_timer = timeGetTime();
	if(fps_timer - fps_timer_old >= FPS_UPDATE)
	{
...
//	R_UpdateScreen
//	Draws all the stuff caled every frame
