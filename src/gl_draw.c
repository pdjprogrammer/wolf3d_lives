...
void R_DrawCrosshair(int bonusflash)
{
	...
	float scale_factor = 1.0f + crosshair_bonus_scale->value * bonusflash / BONUS_FLASH_MAX;

	GLint outer_radius = (GLint)crosshair_outer_radius->value * (GLint)scale_factor;
	GLint inner_radius = (GLint)crosshair_inner_radius->value * (GLint)scale_factor;
