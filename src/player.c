...
void PL_KnifeAttack(player_t *self)
{
	...
	{
		if(Guards[n].flags & FL_SHOOTABLE)
		{
...
void PL_GunAttack(player_t *self)
{
	...
	if(!closest)
	{
		...
		if(trace.flags & TRACE_HIT_DOOR)
			SD_PlaySound(SHOOTDOORSND, CHAN_AUTO);
		return;
	}
...
bool PL_TryMove(player_t *self)
{
	...
	for(y = yl; y <= yh; y++)
		for(x = xl; x <= xh; x++)
	{
		if(CurMapData.tile_info[x][y] & TILE_IS_SOLIDTILE)
			return 0;
		if(CurMapData.tile_info[x][y] & TILE_IS_DOOR && Door_Opened(x, y) != DOOR_FULLOPEN)
			return 0;
	}
...
void PL_Process(player_t *self, usercmd_t *cmd)
{
	...
	if(self->artifacts[ARTIF_INVUL])
	{
		...
		if(self->artifacts[ARTIF_INVUL] < 0)
			self->artifacts[ARTIF_INVUL] = 0;
	}
	if(self->artifacts[ARTIF_INVIS])
	{
		self->artifacts[ARTIF_INVIS] -= tics;
		if(self->artifacts[ARTIF_INVIS] < 0)
			self->artifacts[ARTIF_INVIS] = 0;
	}
	if(self->artifacts[ARTIF_VISION])
	{
		self->artifacts[ARTIF_VISION] -= tics;
		if(self->artifacts[ARTIF_VISION] < 0)
			self->artifacts[ARTIF_VISION] = 0;
	}
}
...
void PL_Damage(player_t *self, Guard_struct *attacker, int points)
{
	...
	if(self->artifacts[ARTIF_INVUL] || self->flags & PL_FLAG_GOD)
		return;
	...
	if(self->armor)
	{
		...
		if((self->armor & ARMOR_MASK) <= saved)
		{
...
bool PL_GiveAmmo(player_t *self, int type, int ammo)
{
	...
	if(self->ammo[type] >= max)
		return false;
	if(!self->ammo[type] && !self->attackframe)
		self->weapon = self->pendingweapon;
...
void PL_GiveWeapon(player_t *self, int weapon)
{
	...
	PL_GiveAmmo(self, AMMO_BULLETS, 6);
...
{
	if(points < (self->armor & ARMOR_MASK))
		return false;
	if(mega)
